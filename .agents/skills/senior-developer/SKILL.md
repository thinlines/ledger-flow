...
**CSS and Tailwind** — Tailwind v4 utility-first. For frontend tasks that touch styling, read `resources/css-rules.md` (in this skill's directory) for the full ruleset on when scoped `<style>` is acceptable, naming conventions, and arbitrary-value policy.
