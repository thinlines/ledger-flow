...
Feature 9 (manual transaction merging + match-ranking fix) is the active focus. Feature 8 (statement reconciliation) is closed through its core track: 8a–8f all shipped, including duplicate review + durable resolution (8d), subset-sum diagnostic (8e), and assertion rendering across surfaces (8f). Phased follow-ups 8g–8j (reconciliation history, statement PDF, adjustment posting, pre-reconciliation edit confirmation) are tabled until a concrete need surfaces. Feature 7 is closed (direction panel, transactions screen rethink through 7d-4b, shell/copy polish). Feature 5 is closed: semantic undo + toast (5e) shipped in full. Feature 9 addresses the import-side root cause that surfaced during 8b user testing: the unknowns review queue's match-suggestion heuristic accepts implausible candidates, and there is no generic merge UI on the transactions page. See `DECISIONS.md` §14–§17, §19–§20 and `plans/statement-reconciliation.md`.
...
8. **Statement reconciliation (Feature 8)** — current focus. Pulled forward from Deferred. Camp 1 (Quicken/YNAB-style explicit reconciliation) over a journal-native balance-assertion substrate. See [`plans/statement-reconciliation.md`](plans/statement-reconciliation.md). Re-sequenced 2026-04-28 after user testing of the 8b modal: the modal worked but cramped the work, and offered no recourse when ledger rejects an assertion despite the user's ticked-diff being zero (the duplicate-in-journal case). 8c now replaces the modal with a route; 8d adds an attest-anyway recourse; subset-sum (8e) is pulled forward; rendering work (8f) moves after the route. Sub-features:
   ...
   - ~~**8b. Reconciliation modal on `/accounts`**~~ — shipped, **superseded by 8c**. Two-step `bits-ui` Dialog on every tracked asset/liability card: dates + closing balance, then a live Opening · Ticked · Closing · Difference strip with checkbox rows from `GET /api/accounts/{id}/reconciliation-context`. Finish posts to 8a; 422/409/400 stayed open with a banner. Currency parser shared with the backend through a JSON fixture and a Vitest parity test. The modal worked for simple statements but cramped multi-month work and lacked recourse for duplicate-in-journal cases. 8c replaces the modal with a dedicated route and removes the modal code.
   - ~~**8a-fix. Date format standardization (ISO 8601)**~~ — shipped. `LEDGER_DATE_FORMAT="%Y-%m-%d"` exported in the runner env, every writer emits `date.isoformat()`, the live workspace was migrated, and a regression test scans fixtures for slash drift. Real-data probe confirmed the Wells Fargo Credit Card `$-1,491.71` reconciliation now lands at the correct file position. See `DECISIONS.md` §20 and the 8a-fix Implementation Notes in `plans/statement-reconciliation.md`.
   - ~~**8c. Reconciliation route + diff-prominent error**~~ — shipped. Replaced the 8b modal with a dedicated `/accounts/:id/reconcile` route. Two entry points: existing Reconcile button on `/accounts` cards (now `goto`s the route), plus a new top-right Reconcile button on `/transactions` when the active filter resolves to a single tracked balance-sheet account (single-account-scope reuse of the totals-strip detection from 7d-4c). On 422, the rejection panel leads with `Off by $X.XX` (absolute magnitude of `closing − actual`), `Your statement: <expected> · Journal: <actual>` subtitle (signed via `signMode: 'good-change-plus'`), and the raw ledger error verbatim behind a `View details` disclosure. Successful reconcile navigates to `/transactions?accounts=<id>` and invalidates the accounts list. The 8b modal component and its bottom-sheet variant code are removed.
   - ~~**8d. Duplicate review + durable resolution**~~ — shipped. Zero-ticked-diff-but-assertion-fails case: the reconcile route surfaces *possible duplicate* groups pairing checked rows with matching unchecked rows using a tightened heuristic (exact amount, normalized payee similarity, narrow date window). Source-aware resolution: `Remove manual duplicate`, `Use imported transaction`, `Merge imported duplicates`. Pulls forward the minimum merge substrate from 9b for reconciliation durability. See `reconciliation_duplicate_service.py`.
   - ~~**8e. Subset-sum “find the difference” diagnostic**~~ — shipped. Bounded combinatoric search (≤ 5 transactions, ≤ 1 s) over unticked rows for sums equal to the missing diff; one-click “Tick these N to close the gap.”
   - ~~**8f. Assertion rendering across surfaces**~~ — shipped. Muted assertion-row style in `/transactions`; `Last reconciled: <date>` on account cards (replaced with red failure copy when broken); loose-ends entry on the dashboard.

   Tabled follow-ups: **8g** reconciliation history, **8h** statement PDF linking, **8i** adjustment-transaction button, **8j** pre-reconciliation edit confirmation. Revisit when a concrete need surfaces.
9. **Manual transaction merging + match-ranking fix (Feature 9)** — current focus. Surfaced by 8b user testing: a duplicate transaction in the journal (caused by the bank changing payee description between imports) blocks reconciliation, and the existing "match" mode in the unknowns review queue suggested an unrelated payee with a >$400 amount delta. 8d shipped the reconciliation-scoped merge substrate and a tightened duplicate heuristic; Feature 9 fixes the unknowns-side heuristic and builds the generic merge UI. Two sub-features:
   - ~~**9a. Match-suggestion ranking fix**~~ — shipped. Replaced the 4-tier (`date_exact_amount` / `date_close_amount` / `date_payee` / `payee_only`) system with continuous scoring in `find_match_candidates()`: amount tolerance (5% of smaller value, capped $5), normalized token-set payee similarity (shared `payee_similarity.py` module), and date decay over a 3-day window. Combined score (weights: 0.50 amt / 0.35 payee / 0.15 date) must clear `MIN_MATCH_SCORE` (0.40) — requires at least two non-zero signals. Auto-suggestion gated on `AUTO_SUGGEST_THRESHOLD` (0.80). Frontend labels updated to "Strong / Likely / Possible match" with per-candidate reason strings. Payee normalization shared with the reconciliation duplicate heuristic via the extracted module.
   - **9b. Manual transaction merge** — current. Full generic merge workflow on the transactions page. Multi-select + Merge action to merge two transactions in the journal into one, preserving both `source_identity` and `source_payload_hash` values on the survivor so future imports of either payee variant are recognized as duplicates. Backend generalizes the reconciliation merge substrate from `reconciliation_duplicate_service.py`; emits a structured event for undo. 8d's merge path is reconciliation-scoped; 9b exposes it as a general-purpose tool. See `plans/transaction-merge.md`.
10. **Interactive dashboard (Feature 10)** — queued, after 9b. Power BI-style dashboard with Apache ECharts charts, cross-filter state, time-hierarchy drill-down (month → week → day), and a global date range picker. Replaces CSS-bar charts with interactive visuals. Backend exposes pre-aggregated `categoryHistory[]` and `cashFlowHistory[]`; individual transaction drill-through uses a scoped lazy fetch rather than embedding raw data in the payload. Four sub-features:
   - **10a. Backend history payload + mtime cache** — expose full `categoryHistory[]` and `cashFlowHistory[]` from already-computed data; add mtime-based transaction cache; add `GET /api/dashboard/transactions` lazy-fetch endpoint.
   - **10b. ECharts cash flow chart + drill state** — install ECharts; replace CSS cash flow bars with grouped bar chart; add `DrillState` and breadcrumb.
   ...
   - **10d. Global date range picker** — bits-ui `DateRangePicker` (zero install — `@internationalized/date` already in devDeps); preset buttons; all `$derived` data filters reactively.
   See `plans/interactive-dashboard.md` for architecture decisions, API contracts, and ECharts integration patterns.
11. **Transaction editing** — deferred. See Deferred for Now.
