...
9. **Manual transaction merging + match-ranking fix (Feature 9)** — current focus. Surfaced by 8b user testing: a duplicate transaction in the journal (caused by the bank changing payee description between imports) blocks reconciliation, and the existing "match" mode in the unknowns review queue suggested an unrelated payee with a >$400 amount delta. 8d shipped the reconciliation-scoped merge substrate and a tightened duplicate heuristic; Feature 9 fixes the unknowns-side heuristic and builds the generic merge UI. Two sub-features:
   - **9a. Match-suggestion ranking fix** — current. The unknowns review queue's `find_match_candidates()` uses a 4-tier system where tier 2 (`date_close_amount`) accepts candidates with implausible deltas and tier 4 (`payee_only`) surfaces payee-substring matches with no date or amount constraint. Replace with a continuous scoring model: normalized payee similarity (reuse 8d's token-set approach), amount tolerance (percentage of smaller value or small absolute cap), date proximity decay. Reject candidates below a minimum combined score. The 8d reconciliation heuristic already demonstrates the stricter approach; 9a backports that quality to the unknowns flow.
   - **9b. Manual transaction merge** — queued, after 9a. Full generic merge workflow on the transactions page. Multi-select + Merge action to merge two transactions in the journal into one, preserving both `source_identity` and `source_payload_hash` values on the survivor so future imports of either payee variant are recognized as duplicates. Backend generalizes the reconciliation merge substrate from `reconciliation_duplicate_service.py`; emits a structured event for undo. 8d's merge path is reconciliation-scoped; 9b exposes it as a general-purpose tool.
