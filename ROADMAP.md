...
7. **Dashboard insight loop and financial direction** — current focus. Four sub-features:
   ...
   - **7d. Transactions screen rethink** — current task. Collapse the dual-mode transactions page into one Monarch-shaped screen with account scope as a filter, an inline category combobox, a right-side detail sheet, the N-1 posting rule, and tracked-to-tracked transfer-pair row collapse. Running balance is always shown. See `plans/transactions-rethink.md`. Ships before 7b because it changes the substrate the rest of Feature 7 builds on. Phases 1–3 are shipped (component extraction, unified row, detail sheet + category combobox + notes). Phase 4 is split into three concurrent-ready sub-tasks:
     ...
     - **7d-4b. Frontend unification** — **current active task.** Rewrite `+page.svelte` to one filter-driven screen. New `TransactionsFilterBar`, `TransactionsFilterDialog`, `transactionFilters.ts`, `loadTransactions.ts`. Remove `activityMode` toggle. Single-account features auto-activate. URL migration for old params. Depends on 7d-4a (shipped).
