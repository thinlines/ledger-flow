...
Statement reconciliation (Feature 8) is the active focus, with Feature 9 (manual transaction merging + match-ranking fix) running alongside as a sibling track. Feature 7 is closed (direction panel, transactions screen rethink through 7d-4b, shell/copy polish). Feature 5 is closed: semantic undo + toast (5e) shipped in full — six undoable mutations (delete, recategorize, unmatch, manual-entry-create, notes-update, status-toggle), the 8s toast on the five destructive ones (status-toggle stays toast-less by design), and a `Recent activity` sheet listing the 20 newest events with per-row Undo / Undone controls, reached via a History trigger in the desktop sidebar and mobile top bar. Feature 8 backend (8a), reconciliation modal MVP (8b), ISO date standardization (8a-fix), and the route conversion + diff-prominent error (8c) are shipped. User testing of 8b uncovered cramped UI for multi-month work, no recourse when the assertion fails despite a zero ticked-diff (the duplicate-in-journal case), error copy from ledger's perspective rather than the user's, and a related bug in the unknowns review queue's match suggestions. Feature 8 is re-sequenced: 8c replaced the modal with a dedicated route (`/accounts/:id/reconcile`) and a `Off by $X.XX` rejection panel; 8d (attest-anyway recourse, current) and 8e (subset-sum diagnostic, pulled forward) follow; assertion rendering across surfaces moves to 8f. Feature 9 addresses the import-side root cause that surfaced the duplicates. See `DECISIONS.md` §14–§17, §19–§20 and `plans/statement-reconciliation.md`.
...
8. **Statement reconciliation (Feature 8)** — current focus. Pulled forward from Deferred. Camp 1 (Quicken/YNAB-style explicit reconciliation) over a journal-native balance-assertion substrate. See [`plans/statement-reconciliation.md`](plans/statement-reconciliation.md). Re-sequenced 2026-04-28 after user testing of the 8b modal: the modal worked but cramped the work, and offered no recourse when ledger rejects an assertion despite the user's ticked-diff being zero (the duplicate-in-journal case). 8c now replaces the modal with a route; 8d adds an attest-anyway recourse; subset-sum (8e) is pulled forward; rendering work (8f) moves after the route. Sub-features:
   ...
   - **8d. Duplicate review + durable resolution** — current focus. Handles the zero-ticked-diff-but-assertion-fails case, which is usually a duplicate-in-journal problem rather than a “delete everything remaining” problem. The reconcile route keeps `Remaining` / `Checked` / `All` review filters, and on zero-diff 422 surfaces *possible duplicate* groups pairing checked rows with matching unchecked rows using a tightened heuristic slice from the unknowns/manual-match flow. Resolution is source-aware: manual-only duplicates can be removed; checked-manual + unchecked-import pairs offer `Use imported transaction`; imported/imported pairs offer `Merge imported duplicates`. This task pulls forward the minimum merge substrate needed for reconciliation durability, but not the full transactions-page merge UI. Generic adjustment posting stays in 8i.
   - **8e. Subset-sum "find the difference" diagnostic** — queued. Pulled forward from old 8f. For the non-zero ticked-diff case. Bounded combinatoric search (≤ 5 transactions, ≤ 1 s) over unticked rows for sums equal to the missing diff; presents top N candidates as one-click "Tick these N to close the gap." Inspired by YNAB Toolkit Reconciliation Assistance.
   - **8f. Assertion rendering across surfaces** — queued. Was old 8c, sequenced after the route lands. Muted assertion-row style in `/transactions`; `Last reconciled: <date>` on account cards (replaced with red failure copy when broken); loose-ends entry on the dashboard.

   Phased follow-ups: **8g** reconciliation history (was 8e), **8h** statement PDF (was 8d), **8i** adjustment-transaction button (was 8g), **8j** pre-reconciliation edit confirmation (was 8h).
9. **Manual transaction merging + match-ranking fix (Feature 9)** — current focus, sibling to Feature 8. Surfaced by 8b user testing: a duplicate transaction in the journal (caused by the bank changing payee description between imports) blocks reconciliation, and the existing "match" mode in the unknowns review queue suggested an unrelated payee with a >$400 amount delta. Two sub-features:
   - **9a. Match-suggestion ranking fix** — bug. The "close amount" classifier in the unknowns review queue accepts candidates with implausible deltas. Tighten: amount delta must be within a fixed percentage of the smaller value or a small absolute tolerance; reject candidates below a minimum combined score (date proximity + amount proximity + payee similarity). This heuristic fix is also a prerequisite slice for 8d's reconciliation duplicate-review surface.
   - **9b. Manual transaction merge** — full generic merge workflow after the reconciliation-scoped merge substrate. Affordance on the transactions page (multi-select + Merge action) to merge two transactions in the journal into one, preserving both `source_identity` and `source_payload_hash` values on the survivor so future imports of either payee variant are recognized as duplicates. Backend writes the merged transaction, deletes the originals, emits a structured event for undo. 8d pulls forward the minimum merge substrate for reconciliation; 9b finishes the generic transactions-page UX.
