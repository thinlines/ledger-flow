...
- **PR 2 — sweep**: in progress
  ...
  - [x] Convert `_undo_transaction_notes_updated`. Added `"transaction.notes_updated.v1"` to `_WRITER_HANDLERS`. Handler signature now `(config: AppConfig, event: dict) -> str`; reads journal, locates header + block, finds (or inserts/removes) the `; notes:` line, and writes the journal — all inside the `mutate` block. Pre-block validation (missing `previous_notes` key, empty `journal_path`/`header_line`) stays outside `mutate` so payload-shape rejection neither backs up nor emits. `backup_file(..., "undo")` removed.
  - [x] Convert `_undo_transaction_unmatched` (multi-file: main journal + archive). Added `"transaction.unmatched.v1"` to `_WRITER_HANDLERS`. Handler signature now `(config: AppConfig, event: dict) -> str`; declares both `journal_path` and `archive_path` in `mutate(paths=…)` so the writer captures pre-mutation existence + hashes for both, re-stamps the imported transaction (`:manual:` + `match-id:` tags + posting rewrite from `Expenses:Unknown` back to original category), removes the restored manual entry from the main journal, and calls `archive_manual_entry` to re-create the archive entry — all inside the `mutate` block. Pre-block validation (payload-shape rejection, missing category in `restored_manual_block`) stays outside `mutate`. `backup_file(..., "undo")` calls for both files removed; unused `backup_file` and `rel_path` imports dropped.
  - [x] Convert `transactions_toggle_status` route (first forward-mutation route conversion). Wrapped read + header locate + write in `with journal_writer.mutate(paths=[journal_path], tag="toggle-status", event_type="transaction.status_toggled.v1") as mut:`. Pre-block validation (journal file existence, `parse_header`, `set_header_status` round-trip via `_HEADER_RE` check) stays outside `mutate` so invalid input neither backs up nor emits. `check_drift`/`hash_file`/`emit_event` wiring removed — writer owns drift, hashing, and emission. `backup_file(..., "toggle")` was never present in this route's body (it pre-existed without the dance); now the writer fully owns it. Swallowed-`emit_event` try/except dropped — emission failures are now real errors per the writer's contract. Returns `mut.event_id`.
  - [x] Convert `transactions_delete` route. Wrapped read + header locate + block find + splice + write in `with journal_writer.mutate(paths=[journal_path], tag="delete", event_type="transaction.deleted.v1") as mut:`. Pre-block validation (journal file existence → 404) and header parse (for summary payee/date) stay outside `mutate`. `check_drift`/`backup_file`/`hash_file`/`emit_event` wiring removed — writer owns drift, hashing, backup, and emission. Swallowed-`emit_event` try/except dropped — emission failures are now real errors per the writer's contract. Returns `mut.event_id`.
  - [x] Convert `transactions_recategorize` route. Wrapped read + header locate + block find + posting walk + rewrite + write in `with journal_writer.mutate(paths=[journal_path], tag="recategorize", event_type="transaction.recategorized.v1") as mut:`. Pre-block validation (journal file existence → 404, header parse for summary payee/date, tracked-account ledger-name collection from config) stays outside `mutate`. File-content-dependent 422 rejections (already uncategorized, split transaction, transfer, already has this category) and the 500 "Failed to rewrite posting account" stay **inside** the block — they raise `HTTPException`, escape the block, and the writer rolls back from backup (a no-op restore since these checks fire before the journal `write_text`). `check_drift`/`backup_file`/`hash_file`/`emit_event` wiring removed. Swallowed-`emit_event` try/except dropped. Returns `mut.event_id`.
  - [x] Convert `transactions_reassign_account` route. Wrapped read + header locate + block find + source-posting walk + rewrite + write in `with journal_writer.mutate(paths=[journal_path], tag="reassign-account", event_type="transaction.account_reassigned.v1") as mut:`. Pre-block validation (journal file existence → 404, header parse for summary payee/date, tracked-account ledger-name collection, "Not a tracked account" 422 on `req.newAccountLedgerName`) stays outside `mutate` — these are all config-only / payload-shape checks that neither back up nor emit. File-content-dependent rejections (422 "Cannot reassign a multi-account transaction", 422 "Cannot reassign account on this transaction", 422 "Transaction already belongs to this account", 500 "Failed to rewrite posting account", 404 from `_locate_header`) stay **inside** the block — they raise `HTTPException`, escape the block, and the writer rolls back from backup. `check_drift`/`backup_file`/`hash_file`/`emit_event` wiring removed. Swallowed-`emit_event` try/except dropped. Returns `mut.event_id`.
  - [x] Convert `transactions_notes` route. Wrapped read + header locate + block find + notes-line search + set/insert/remove + write in `with journal_writer.mutate(paths=[journal_path], tag="notes", event_type="transaction.notes_updated.v1") as mut:`. Pre-block validation (journal file existence → 404, header parse for summary payee/date — falls back to header-line slice on parse miss, no rejection) stays outside `mutate`. 404 from `_locate_header` escapes the block and the writer rolls back. `check_drift`/`backup_file`/`hash_file`/`emit_event` wiring removed — writer owns drift, hashing, backup, and emission. Swallowed-`emit_event` try/except dropped — emission failures are now real errors per the writer's contract. Returns `mut.event_id`.
  - [x] Convert `transactions_unmatch` route (first multi-file forward-route conversion: main journal + `archived-manual.journal`). Wrapped archive locate + main-journal tag-strip + posting rewrite + first write + restored-entry insertion + second write + archive-block removal (or unlink when empty) in `with journal_writer.mutate(paths=[journal_path, archive_path], tag="unmatch", event_type="transaction.unmatched.v1") as mut:`. Pre-block validation (journal file existence → 404, archive file existence → 404, tracked-account ledger-name collection from config, `parse_header` for summary payee/date) stays outside `mutate` — config-only / shape checks that neither back up nor emit. File-content-dependent rejections (404 "Archived manual entry not found for this match", 404 from `_locate_header`) stay **inside** the block — they raise `HTTPException`, escape the block, and the writer rolls back both files from backup. `check_drift`/`backup_file`/`hash_file`/`emit_event` wiring removed (both files) — writer owns drift, hashing, backup, and emission across the pair. Swallowed-`emit_event` try/except dropped. The conditional archive-ref logic (legacy: include only when `archive_path.is_file()` post-mutation) is gone — the writer's `journal_refs` rule (include any path whose hash changed) handles the archive-deleted case naturally: hash_after becomes `sha256:none`, differs from hash_before, ref is included. Returns `mut.event_id`. Redundant inline `from services.transfer_service import ACCOUNT_LINE_RE, ACCOUNT_ONLY_RE` import dropped (already at the module top).
  - [x] Convert `transactions_create` route (manual entry creation). Wrapped the `create_manual_transaction(...)` call in `with journal_writer.mutate(paths=[journal_path], tag="manual-entry", event_type="manual_entry.created.v1") as mut:`. The journal file may not exist pre-mutation (manual entry is the canonical first-write path for a year); the writer's pre_existed/hash_before bookkeeping handles that case via `"sha256:none"` and an unconditional file-delete-on-rollback. Pre-block validation (tracked-account existence → 404) stays outside `mutate` — config-only payload-shape check. `ValueError` from `create_manual_transaction` (invalid amount string, missing tracked ledger account) stays **inside** the block — it raises `HTTPException(400)`, escapes the block, and the writer rolls back (deletes the file if it was newly created, restores from backup otherwise). `check_drift`/`hash_file`/`emit_event` wiring removed — writer owns drift, hashing, and emission. Swallowed-`emit_event` try/except dropped — emission failures are now real errors per the writer's contract. Returns `mut.event_id`.
  - [x] Convert `import_apply` route (CSV-derived bulk-append; target journal may not exist pre-mutation). Wrapped `apply_import` + `archive_inbox_csv` + stage/history bookkeeping in `with journal_writer.mutate(paths=[journal], tag="import", event_type="import.applied.v1") as mut:`. Pre-block validation (stage lookup → 404, stage kind → 400, idempotent `status == "applied"` early-return) stays outside `mutate`. `CommandError` from `apply_import` stays **inside** the block — caught and re-raised as `HTTPException(400)`, escapes the block, writer rolls back (deletes journal if it was newly created, restores from backup otherwise). `OSError` from `archive_inbox_csv` stays inside the block but is captured to `source_csv_warning` (non-fatal; archive failure must not roll back the journal — per PRD §6, archive CSV moves are intentionally outside the writer's scope). `record_applied_import` + `stages.save` happen inside the block so a failure rolls back the journal alongside; `mut.payload` references `history_entry["id"]`, so history must be recorded before payload assembly. `check_drift`/`backup_file`/`hash_file`/`emit_event` wiring removed — writer owns drift, hashing, backup, and emission. Swallowed-`emit_event` try/except dropped — emission failures are now real errors per the writer's contract. `stage["result"]["backupPath"]` is now always `None` — the writer manages backups internally and does not surface their paths. The frontend handles `null` gracefully via `{#if entry.backupPath}` and `record_applied_import` reads `result.get("backupPath")` (None passes through to the history entry).
  - [x] Convert `import_undo` route. Wrapped the `undo_import(config, req.historyId)` call in `with journal_writer.mutate(paths=[journal_path], tag="import-undo", event_type="import.undone.v1") as mut:`. Pre-block validation (resolve `journal_path` from `list_import_history` by `historyId` → 404 if entry or `targetJournalPath` missing) stays outside `mutate` — collapses the legacy "history not found" KeyError + "missing target" cases into a single 404 before backup. `KeyError`/`FileNotFoundError`/`OSError`/`ValueError` from `undo_import` stay **inside** the block — caught and re-raised as `HTTPException(404/400)`, escape the block, writer rolls back from backup. `check_drift`/`hash_file`/`emit_event` wiring removed — writer owns drift, hashing, and emission. Best-effort pre-resolution `try/except Exception` swallow (legacy log-and-continue) dropped — resolution errors now surface as 500s rather than silent skipped emits. Swallowed-`emit_event` try/except dropped — emission failures are now real errors per the writer's contract. Returns `{"entry": entry}` unchanged (`mut.event_id` not surfaced in this route's response; the event id is recoverable via the events log). Undo's internal `backup_file(journal_path, "undo")` inside `undo_import` is intentionally left untouched — harmless duplicate backup, removed when PR 2 sweeps `backup_service.py`; the `undoBackupPath` it populates is still asserted by `test_import_history_service.py` and remains user-facing forensic data on the history entry.
  - [x] Convert `unknown_apply` route (first three-path mutation: journal + `10-accounts.dat` + `archived-manual.journal`; pressure-tests the writer's co-candidate scope from PRD §6). Wrapped `apply_unknown_mappings` + stage status/result mutation + `stages.save` in `with journal_writer.mutate(paths=[journal_path, accounts_dat, archived_manual], tag="unknowns", event_type="unknowns.applied.v1") as mut:`. Pre-block validation (stage lookup → 404, stage kind → 400, idempotent `status == "applied"` early-return, selections-empty → 400, path derivation) stays outside `mutate`. `ValueError` from `apply_unknown_mappings` (unknown account in mapping) stays **inside** the block — caught and re-raised as `HTTPException(400)`, escapes the block, writer rolls back all three paths (journal/accounts_dat byte-restore from backup; archive deleted if newly created, else byte-restore from its backup). `stages.save` stays inside the block so a failure rolls back the journal alongside (consistent with the import_apply ordering rule). `check_drift`/`backup_file`/`hash_file`/`emit_event` wiring removed for all three paths — writer owns drift, hashing, backup, and emission across the trio. Swallowed-`emit_event` try/except dropped. `stage["result"]["backupPaths"]` is now `None` — the writer manages backups internally and does not surface their paths; no test, frontend, or backend reads this field. **Behavioral note**: the legacy route unconditionally emitted both `journal_path` and `accounts_dat` refs (even when accounts_dat was unchanged — e.g. all selections were `match` not `category`), and conditionally appended the archive ref. The writer's `journal_refs` rule (include any path whose hash actually changed) is path-set-agnostic — accounts_dat is now omitted when unchanged, archive is included when its hash changes, all uniformly. Inner `rollback_archive` inside `apply_unknown_mappings` is left untouched — harmless (in-place truncate is dominated by the writer's byte-restore from its pre-mutation backup), removed when PR 2 sweeps `backup_service.py`. Returns `stage` unchanged.
  - [x] Convert `rules_history_apply` route (last remaining non-skipped mutation route). Wrapped `apply_rule_reapply(...)` + stage status/result mutation + `stages.save` in `with journal_writer.mutate(paths=[journal_path], tag="rule-history", event_type="rule.history_applied.v1") as mut:`. Pre-block validation (stage lookup → 404, stage kind → 400, idempotent `status == "applied"` early-return, selections-empty → 400, path derivation) stays outside `mutate`. `ValueError` from `apply_rule_reapply` (unknown account / no selections / missing candidates) stays **inside** the block — caught and re-raised as `HTTPException(400)`, escapes the block, writer rolls back from backup. `stages.save` stays inside the block so a failure rolls back the journal alongside (consistent with the import_apply / unknown_apply ordering rule). `check_drift`/`backup_file`/`hash_file`/`emit_event` wiring removed — writer owns drift, hashing, backup, and emission. Swallowed-`emit_event` try/except dropped. `stage["result"]["backupPath"]` is now `None` — the writer manages backups internally and does not surface their paths. Frontend `unknowns/+page.svelte` updated: type narrowed to `backupPath: string | null`, and the "Backup: …" display line gated behind `{#if historyStage.result.backupPath}` (matches the ImportFlow pattern). Returns `stage` unchanged.
  - [ ] Manual-transfer-resolution-apply skipped (being removed in PR 3 per PRD §Out-of-Scope). Route surface sweep is now complete.
  - [x] Convert `accounts_reconciliation_duplicate_resolution` route — three action branches in `services/reconciliation_duplicate_service.resolve_duplicate_candidate`. This route was added concurrently with PR 1 and so was missing from the original sweep list. Wrapped each branch's mutation in its own `with journal_writer.mutate(...)` block: `remove_manual_duplicate` (single path: `journal_path`, event `reconciliation.duplicate_manual_removed.v1`), `use_imported_transaction` (multi-path: `manual_journal` + `archive_path` + optionally `imported_journal` when distinct, event `reconciliation.imported_transaction_used.v1`), `merge_imported_duplicates` (single path: `journal_path`, event `reconciliation.imported_duplicates_merged.v1`); all three carry `tag="reconcile-duplicate"`. Pre-block validation (tracked-account 404, missing-ledger-account 400, same-journal requirement on merge 422) stays outside `mutate` — config-only / payload-shape checks that neither back up nor emit. File-content-dependent 422s ("Split manual duplicates need a fuller merge flow", "Imported duplicate does not have a usable category posting") and the 409 from `_read_block`'s `locate_header_at` stay **inside** the block — they raise `HTTPException`, escape the block, and the writer rolls back every declared path from backup (archive deletes when newly created). Dropped the manual dance: `_capture_hash_before` + `_backup_once` + `_emit_resolution_event` helpers, the inner `try/except` + `journal_path.write_text(original_text, ...)` rollback blocks, and the `rollback_archive` call. Imports `check_drift`/`emit_event`/`hash_file`/`backup_file`/`rollback_archive` dropped from the service (`rel_path` retained — still used in the `remove_manual_duplicate` payload). The `test_duplicate_resolution_rolls_back_when_event_write_fails` test was removed — its rollback-on-emit-failure assertion is no longer part of the writer's contract (consistent with every other PR 2 conversion: "emission failures are now real errors"). The `from services import reconciliation_duplicate_service` import in the test file is dropped (only used by the removed test). The writer's `journal_refs` rule (include any path whose hash actually changed) naturally handles the `imported_journal == manual_journal` collapse and the conditional archive ref. Returns `mut.event_id` from each branch's `return {... "eventId": mut.event_id}`. Backend tests: 795 passing (one fewer than the prior 796 from the removed rollback-on-emit-failure test).
  - [x] Drop the legacy-handler branch from `undo_event`. Every entry in `_HANDLERS` was already in `_WRITER_HANDLERS`, making the legacy branch dead code. Removed `_WRITER_HANDLERS` frozenset, `LegacyHandlerFn` type alias, the `HandlerFn` union (now a single `Callable[[AppConfig, dict], str]`), the `else` branch in `undo_event` that called legacy handlers + manually emitted via `emit_event`, and the now-unused `emit_event` import + `pathlib.Path` import. Dispatcher is now ~25 lines shorter; every undo handler flows through the same writer-routed protocol. All 804 backend tests pass.
  - [x] Delete `backup_service.py`; drop `restore_from_backup` from `reconciliation_service` public exports. `app/backend/services/backup_service.py` removed. `restore_from_backup` (4-line `shutil.copyfile` wrapper) removed from `reconciliation_service`; `shutil` import dropped (sole consumer). `write_assertion_transaction` signature simplified from `tuple[AssertionWriteResult, Path]` to just `AssertionWriteResult` — the duplicate inner `backup_file(journal_path, "reconcile")` call and the local `from .backup_service import backup_file` import are gone; the writer's pre-mutation backup dominates. `main.py` route updated: `write_result, _backup = ...` → `write_result = ...`. `import_history_service.undo_import` drops `backup_file(journal_path, "undo")` and the `undoBackupPath` field on the history-entry's `undo` dict; the `backup_file` import is removed. Frontend `ImportFlow.svelte` type declaration loses the `undoBackupPath?: string | null` field (it was never displayed). Tests updated: `test_reconciliation_service.py` drops `restore_from_backup` import + the `TestRollback` class + every `result, _ = ...` tuple unpack (now `result = ...`); `test_undo_service.py`, `test_notes_and_recategorize.py`, `test_transaction_actions.py` drop their cosmetic `backup_file(...)` fixture calls and the corresponding inline `from services.backup_service import backup_file` imports (the backups were never read by any assertion — they were warm-up calls modeling the legacy dance); `test_import_history_service.py` drops the two `Path(undone["undo"]["undoBackupPath"]).exists()` / `.read_text()` assertions. `test_journal_writer.py` docstring updated to reference the writer's internal backup helper instead of the deleted `backup_file`. Backend tests: 794 passing (was 795; the `-1` is the deleted `test_restore_from_backup_yields_byte_identical_journal`). Frontend type check: 870 files, 0 errors, 0 warnings. PR 2 sweep is now complete — every forward and undo journal mutation flows through `journal_writer.mutate`, no module outside the writer creates a backup, and the public `backup_service` module is gone.
- **PR 3 — cleanup (manual transfer resolution removal + DECISIONS §22)**: complete
  - [x] Remove the manual transfer resolution feature end-to-end. Deleted `app/backend/services/manual_transfer_resolution_service.py`, `app/backend/tests/test_manual_transfer_resolution_service.py`, and `app/frontend/src/lib/components/transactions/ManualResolutionDialog.svelte`. Dropped both route handlers (`/api/transactions/manual-transfer-resolution/{preview,apply}`) from `main.py`; removed the now-dead `services.backup_service.backup_file` and `services.event_log_service.{check_drift,emit_event,hash_file}` imports from `main.py` (the manual-transfer-resolution-apply route was their last consumer). Dropped `ManualTransferResolutionRequest` from `models.py`. Dropped `MANUAL_TRANSFER_RESOLUTION_METADATA_KEY/_VALUE/_TOKEN_KIND/_TOKEN_VERSION` constants and `build_manual_transfer_resolution_token` / `parse_manual_transfer_resolution_token` functions from `services/transfer_service.py` (only this feature consumed them; the `base64`/`json` imports became dead and were removed). Dropped `manual_resolution_token` / `manual_resolution_note` helpers, the matching `RegisterEvent` fields, and their `pending_transfer_event_for_peer_account` call sites from `services/transaction_helpers.py`. Dropped the `manualResolutionToken`/`manualResolutionNote` response fields and the helper calls from `services/account_register_service.py` and `services/unified_transactions_service.py`. Removed the two manual-resolution-specific tests (`test_account_register_exposes_same_manual_resolution_token_on_source_and_peer_pending_rows`, `test_account_register_adds_manual_resolution_note_for_resolved_transfer_rows`) and their now-unused `MANUAL_TRANSFER_RESOLUTION_METADATA_KEY/VALUE` imports from `test_account_register_service.py`; pruned residual `manualResolutionToken is None` assertions from the surviving bilateral-match tests. Frontend: dropped `ManualResolutionDialog` import + binding from `transactions/+page.svelte`, removed `manualResolutionEntry`/`manualResolutionSuccess` state, the `openManualRes`/`handleResolved` handlers, the "Resolved" success card, and the `{#if row.manualResolutionToken}` "Resolve manually" affordance from the pending-transfers list (pending-details-note collapses to the "Waiting for the imported transaction to land." path). Dropped the deprecated `RegisterEntry`, `ManualResolutionPreview`, `ManualResolutionApplyResult` types and the `manualResolutionToken`/`manualResolutionNote` fields from `TransactionRow` in `lib/transactions/types.ts`. Dropped the `{#if row.manualResolutionNote}` block from `TransactionDetailSheet.svelte`. Recorded DECISIONS §22 capturing the load-bearing design constraint (writer's `paths`-declared-up-front contract is incompatible with the previous "discover-after" glob-all-journals pattern; any future reintroduction must pre-resolve the target path outside the writer and be motivated by a real-world workflow). Backend tests: 796 passing (8 fewer than before — 6 from the deleted service test + 2 from the deleted register tests). Frontend type check: 870 files, 0 errors, 0 warnings.
