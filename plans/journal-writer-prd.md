...
- **PR 2 — sweep**: in progress
  ...
  - [ ] Delete `backup_service.py`; drop `restore_from_backup` from `reconciliation_service` public exports. No longer gated — PR 3 (below) removed the last forward caller (`manual_transfer_resolution_service.py`). Remaining live `backup_file` calls are inside writer blocks (`reconciliation_service.write_assertion_transaction`, `import_history_service.undo_import`) — harmless duplicate backups, dropped alongside the service.
