...
- **PR 2 — sweep**: in progress
  ...
  - [ ] Delete `backup_service.py`; drop `restore_from_backup` from `reconciliation_service` public exports. Gated on PR 3's removal of `manual_transfer_resolution_service.py` (currently the only forward caller that still uses `backup_file` outside of legacy redundant calls in `reconciliation_service.write_assertion_transaction` and `import_history_service.undo_import`, which are inside writer blocks now).
