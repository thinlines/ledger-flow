...
- **PR 2 — sweep**: in progress
  ...
  - [ ] Delete `backup_service.py`; drop `restore_from_backup` from `reconciliation_service` public exports. No longer gated — PR 3 removed `manual_transfer_resolution_service.py` and the `reconciliation_duplicate_service` conversion (above) removed the other forward caller that the original task description missed. Remaining live `backup_file` calls are inside writer blocks (`reconciliation_service.write_assertion_transaction`, `import_history_service.undo_import`) — harmless duplicate backups, dropped alongside the service. Test files (`test_undo_service.py`, `test_notes_and_recategorize.py`, `test_transaction_actions.py`) construct backups directly for fixture setup; those will inline the trivial backup helper or be reworked when the service is deleted.
