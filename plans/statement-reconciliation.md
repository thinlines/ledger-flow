...
- **Modal removal is total.** `ReconcileModal.svelte`, the import, the `reconcileTarget` mount state, the `reconcileRefreshError` banner, the `handleReconciled` hook, and the `reconcileAccountKind` helper all came out of `/accounts/+page.svelte` in one pass. The bottom-sheet variant code paths (`shell:` / `max-shell:` Dialog positioning) went with the file. `git grep "ReconcileModal"` returns only TASK.md and this plan file.
