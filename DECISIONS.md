...
## 20. Journal Dates Are ISO 8601 (`YYYY-MM-DD`)

**Decision:** Every journal file produced or rewritten by Ledger Flow uses ISO 8601 (`YYYY-MM-DD`) on transaction header lines. The ledger CLI is invoked with `LEDGER_DATE_FORMAT="%Y-%m-%d"` exported in its environment so its own output (errors, balance reports, `convert` results) also uses ISO. The runner — `services.ledger_runner.run_cmd` — copies `os.environ`, overlays the format, and passes `env=` to `subprocess.run`, so every call site inherits the setting from a single chokepoint. We deliberately do **not** set `LEDGER_INPUT_DATE_FORMAT`: ledger's default parser still tolerates slash-formatted input, which keeps externally-sourced third-party journals readable without configuration.

**Why:** 8a's `_insertion_index_for_date` compares `line[:10]` against `period_end.isoformat()`. When a journal contained slash-formatted headers, those comparisons silently mismatched, and reconciliation assertions landed at the top of the file instead of after `periodEnd`'s last transaction. The bug surfaced during 8b's first real-data probe (Wells Fargo Credit Card, `$-1,491.71`, `periodEnd = 2026-01-18`). Fixing the symptom in one writer would leave every future date-prefix string comparison fragile — pinning the project to a single canonical format eliminates the whole class of bug.

**Implication:**
- New writers must emit `date.isoformat()`. The regression test `app/backend/tests/test_journal_date_format.py` scans committed test fixtures for slash headers and fails the build if any drift back.
...
## 22. Manual Transfer Resolution Is Not Preserved Across the `journal_writer` Deepening

**Decision:** The manual transfer resolution feature (route, service, dialog, model, dedicated tests, and per-event metadata key) is removed in full and not reintroduced inside the `journal_writer` chokepoint. Any future reintroduction must pre-resolve the target journal path *outside* the writer and arrive at `journal_writer.mutate(paths=[…])` with a concrete `paths` list, and must be justified by a real-world use case rather than a speculative one.

**Why:** The previous design relied on "discover-after" path resolution — the apply route accepted a resolution token, walked every `*.journal` file under `journal_dir` to locate the source transaction, then mutated whichever file the token resolved into. That shape is fundamentally incompatible with the writer's `paths`-declared-up-front contract (Implementation Decision §2 of `plans/journal-writer-prd.md`). Preserving the feature would either (a) bypass the writer and re-open the chokepoint we just closed, or (b) force the writer to support a worst-case "any journal might be touched" path set — backing up every year journal on every call, polluting `journal_refs`, and degrading the contract for every other op. Neither was acceptable, and the feature had no proven real-world traffic to justify either compromise.

**Implication:** The endpoints `/api/transactions/manual-transfer-resolution/preview` and `/api/transactions/manual-transfer-resolution/apply` are gone. `ManualResolutionDialog.svelte`, `manual_transfer_resolution_service.py`, and `test_manual_transfer_resolution_service.py` are deleted. The `transfer_manual_resolution` journal metadata key and its `missing_import_counterpart` value are no longer emitted or recognized; the `manualResolutionToken` / `manualResolutionNote` fields are removed from `/api/transactions` and `/api/accounts/<id>/register` response shapes. Pending import-match transfers without a counterpart will sit in the pending list until either the counterpart imports or the user reverses the source transaction. A future reintroduction is allowed: the candidate design must surface a concrete `journal_path` to the route handler before calling the writer (e.g., by carrying the source transaction's journal path in the token or by resolving it via a non-glob index), and must cite a user workflow that motivates the feature beyond completeness.

**References:** [`plans/journal-writer-prd.md`](plans/journal-writer-prd.md) (full design context, §Out-of-Scope), `plans/journal-writer-progress.txt` (sweep history), §3 (workspace canonical / `.workflow/` boundary), §12 (event-sourced undo).
